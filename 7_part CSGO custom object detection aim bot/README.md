...
import pyautogui
```
This library will be used to move our mouse in game. But some games may not allow you to move mouse, then you will need to start python script with administrator rights, same as I am doing for CSGO.

Next we are changing defined monitor size line to following. We are doing this because we will need to use our window width and height in other places to calculate right coordinates for our game. So to avoid mistakes and not to write same values in many places, we are defining our window size accordingly:
```
width = 800
height = 640
...
```
After this we are making functions what we want to shoot. So as team = “t” we choose who we will be shooting at, at this case we are shooting terrorists. So at first we check if we have detected terrorists heads, if we have at least one head, we call Shoot(mid_x, mid_y) function with needed coordinates. If we can’t detect any heads we check maybe we detected terrorists bodies, if we did, we call the same shooting function.
